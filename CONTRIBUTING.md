...
Struttura attesa: radice con `ignite64py/`, `tb_Ignite64/`, `README.md`, `AGENTS.md`, `.gitignore`.

### Ambiente Python

Seguire **[ignite64py/docs/INSTALLAZIONE_WINDOWS.md](ignite64py/docs/INSTALLAZIONE_WINDOWS.md)** (venv, `pip install -e ignite64py/` o `cd ignite64py && pip install -e .`).

### DLL

Posizionare `TCPtoI2C.dll` e `USBtoI2C32.dll` come da documentazione (cartella `ignite64py/` o `PATH`). Non vengono committate di default (`.gitignore` include `*.dll`). In un fork **privato**, se la licenza lo consente, si può rimuovere o commentare quella riga in `.gitignore`.
...
| `ConfigurationFiles/` testo, asset immagini in `ignite64py/ignite64py/assets/` | DLL vendor (salvo policy diversa) |
